// Program to implement Page Replacement LRU algorithm
...
void pageLRU() //To implement the algorithm
{
    int flag, hit_miss;
    for (int i = 0; i < num_pages; i++)
    {
        ...
        {
            int lru = pages[0].index; //Index of Least Recently used page
            int lpage = 0;            //Least Recently used page
            for (int k = 1; k < frames; k++)
            {
                if (pages[k].index < lru)
                {
                    lru = pages[k].index;
                    lpage = k;
                }

//Program to implement Shortest Job First scheduling algorithm in C
#include <stdio.h>

struct PCB
{
    int pid, bt, ct, wt, tat;
    //process id, burst time, completion time, waiting time, turn around time
};
struct PCB p[10];

void insert(int n) //To input the burst times
{
    for (int i = 0; i < n; i++)
    {
...
void sort(int n) //To sort the processes based on the burst time
{
    struct PCB temp;
    for (int i = 0; i < n; i++)
    {
